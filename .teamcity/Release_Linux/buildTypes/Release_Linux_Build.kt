...
object Release_Linux_Build : Template({
    name = "Build"
    ...
    steps {
        ...
        script {
            name = "Smoke test"
            id = "RUNNER_57"
            scriptContent = "scripts/test/smoke-test.sh dist/quasardb*.whl"
            dockerImage = "build:%build.vcs.number%-%PYTHON_VERSION%"
            dockerImagePlatform = ScriptBuildStep.ImagePlatform.Linux
        }
        exec {
    ...
    requirements {
        doesNotContain("teamcity.agent.jvm.os.arch", "aarch64", "RQ_134")
        doesNotContain("teamcity.agent.jvm.os.arch", "arm", "RQ_304")
    }
})
