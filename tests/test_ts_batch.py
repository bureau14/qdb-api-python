...
def _row_insertion_method(batch_inserter, dates, doubles, blobs, integers, timestamps):
...
def test_non_existing_bulk_insert(qdbd_connection, entry_name):
    with pytest.raises(quasardb.Error):
        qdbd_connection.ts_batch([quasardb.BatchColumnInfo(entry_name, "col", 10)])

def _test_with_table(batch_inserter, table, intervals, insertion_method, push_method = _regular_push):
    ...
    insertion_method(batch_inserter, intervals, doubles, blobs, integers, timestamps)
...
def test_successful_bulk_row_insert(qdbd_connection, table, many_intervals):
    ...
    _test_with_table(batch_inserter, table, many_intervals, _row_insertion_method, _regular_push)
...
def test_successful_async_bulk_row_insert(qdbd_connection, table, many_intervals):
    batch_inserter = qdbd_connection.ts_batch(_make_ts_batch_info(table))
    _test_with_table(batch_inserter, table, many_intervals, _row_insertion_method, _async_push)
