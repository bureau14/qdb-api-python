...
def _generate_string_ts(start_time, count):
    dates = _generate_dates(start_time, count)
    values = np.array(
        [("content_" + str(item)) for item in range(count)], dtype=np.dtype("U")
    )
...
def test_list_columns_throws_when_table_does_not_exist(qdbd_connection, entry_name):
    table = qdbd_connection.table(entry_name)
    with pytest.raises(quasardb.AliasNotFoundError):
        table.list_columns()


def test_has_ttl_throws_when_table_does_not_exist(qdbd_connection, entry_name):
    table = qdbd_connection.table(entry_name)
    with pytest.raises(quasardb.AliasNotFoundError):
        table.has_ttl()


def test_get_ttl_throws_when_table_does_not_exist(qdbd_connection, entry_name):
    table = qdbd_connection.table(entry_name)
    with pytest.raises(quasardb.AliasNotFoundError):
        table.get_ttl()
...
def test_has_ttl_returns_false_when_no_ttl_set(qdbd_connection, entry_name):
    table = qdbd_connection.table(entry_name)
    table.create([])
...
def test_get_ttl_returns_zero_when_no_ttl_set(qdbd_connection, entry_name):
    table = qdbd_connection.table(entry_name)
    table.create([])
...
def test_get_ttl_returns_ttl_when_ttl_set(qdbd_connection, entry_name, shard_size, ttl):
    table = qdbd_connection.table(entry_name)
    table.create([], shard_size=shard_size, ttl=ttl)
...
def test_table_layout(table):
    ...
    """
    expected = [
        ("the_double", quasardb.ColumnType.Double),
