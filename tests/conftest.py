...
def create_many_intervals():
    start_time = np.datetime64("2017-01-01", "ns")
    return np.array(
        [(start_time + np.timedelta64(i, "s")) for i in range(10000)]
...
@pytest.fixture(
    params=[
        quasardb.WriterPushMode.Fast,
        quasardb.WriterPushMode.Transactional,
        quasardb.WriterPushMode.Async,
    ],
    ids=["fast", "transactional", "async"],
)
def push_mode(request):
    yield request.param


@pytest.fixture(
    params=[1, 60, 3600, 86400],
    ids=["shard_size=1s", "shard_size=1min", "shard_size=1h", "shard_size=1d"],
)
...
@pytest.fixture(
    params=[1, 60, 3600, 86400, 604800, 7776000],
    ids=["ttl=1s", "ttl=1min", "ttl=1h", "ttl=1day", "ttl=1week", "ttl=90days"],
)
def ttl(request):
    import datetime
...
def _array_with_index_and_table(
    qdbd_connection, table_name, column_name, ctype, dtype, data, start_date, row_count
):
...
@pytest.fixture
def array_with_index_and_table(
    qdbd_connection, table_name, column_name, gen_array, start_date, row_count
):
    (ctype, dtype, array) = gen_array

    return _array_with_index_and_table(
        qdbd_connection,
        table_name,
        column_name,
        ctype,
...
@pytest.fixture
def arrays_with_index_and_table(
    qdbd_connection, table_name, column_name, gen_arrays, start_date, row_count
):
...
@pytest.fixture(
    params=["bool", "column_name", "$timestamp"],
    ids=[
        "deduplicate=True",
...
@pytest.fixture(
    ...
    ids=["int64", "double", "blob", "string", "timestamp", "symbol"],
)
...
@pytest.fixture(
    params=timedeltas,
    ids=["delta={}s".format(int(x.total_seconds())) for x in timedeltas],
)
...
query_fns = {"query_numpy": _query_style_numpy, "query_regular": _query_style_regular}


@pytest.fixture(params=["query_numpy", "query_regular"])
...
@pytest.fixture(
    params=[1, 2, 4], ids=["retry_exponent=1", "retry_exponent=2", "retry_exponent=4"]
)
...
def retry_options(retry_count, retry_delay, retry_exponent):
    return quasardb.RetryOptions(
        retries=retry_count, delay=retry_delay, exponent=retry_exponent
    )


@pytest.fixture(
    params=[0, 1, 2], ids=["failure_count=0", "failure_count=1", "failure_count=2"]
)
