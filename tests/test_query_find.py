...
def test_types_tag(qdbd_connection, blob_entry, random_blob, tag_name):
    ...
    res = qdbd_connection.find("find(tag='" + tag_name + "')")

    assert len(res) == 1

    res = qdbd_connection.find("find(tag='" + tag_name + "' AND type=blob)")

    assert len(res) == 1

    res = qdbd_connection.find("find(tag='" + tag_name + "' AND type=integer)")
...
def test_types_tags(qdbd_connection, blob_entry, random_blob, tag_names):
    ...
    res = qdbd_connection.find("find(tag='" + tag_names[0] + "')")
    assert len(res) == 1

    res = qdbd_connection.find("find(tag='" + tag_names[1] + "')")
    assert len(res) == 1
