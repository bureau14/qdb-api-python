...
def test_list_columns_throws_when_timeseries_does_not_exist(qdbd_connection, entry_name):
...
def test_get_ranges_throws_when_timeseries_does_not_exist(qdbd_connection, entry_name, intervals):
    ts = qdbd_connection.ts(entry_name)
    with pytest.raises(quasardb.Error):
        ts.double_get_ranges("blah", intervals)

def test_erase_ranges_throw_when_timeseries_does_not_exist(qdbd_connection, entry_name, intervals):
    ts = qdbd_connection.ts(entry_name)
    with pytest.raises(quasardb.Error):
...
def test_create_with_shard_size_less_than_1_millisecond_throws(qdbd_connection, entry_name):
    ts = qdbd_connection.ts(entry_name)
    with pytest.raises(quasardb.Error):
...
def test_cannot_double_create(table, entry_name):
    with pytest.raises(quasardb.Error):
        table.create([quasardb.ColumnInfo(quasardb.ColumnType.Double, entry_name)])
...
def test_double_get_ranges(table, intervals):
    ...
    with pytest.raises(quasardb.Error):
        table.double_get_ranges("lolilol", [(_start_time(intervals), _start_time(intervals) + np.timedelta64(10, 's'))])

    with pytest.raises(quasardb.Error):
        table.double_insert("lolilol", inserted_double_data[0], inserted_double_data[1])

    with pytest.raises(TypeError):
        table.blob_get_ranges(_double_col_name,
                              [(_start_time(intervals), _start_time(intervals) + np.timedelta64(10, 's'))])

    with pytest.raises(quasardb.Error):
        table.blob_insert(_double_col_name(table), inserted_double_data[0], inserted_double_data[1])


def test_double_erase_ranges(table, intervals):
    inserted_double_data = _generate_double_ts(_start_time(intervals), 1000)
    table.double_insert(_double_col_name(table), inserted_double_data[0], inserted_double_data[1])
...
def test_int64_get_ranges(table, intervals):
    ...
    # error: column doesn't exist
    with pytest.raises(quasardb.Error):
        table.int64_get_ranges("lolilol", [(_start_time(intervals), _start_time(intervals) + np.timedelta64(10, 's'))])

    with pytest.raises(quasardb.Error):
        table.int64_insert("lolilol", inserted_int64_data[0], inserted_int64_data[1])

    with pytest.raises(TypeError):
        table.blob_get_ranges(_int64_col_name,
                              [(_start_time(intervals), _start_time(intervals) + np.timedelta64(10, 's'))])

    with pytest.raises(quasardb.Error):
        table.blob_insert(_int64_col_name(table), inserted_int64_data[0], inserted_int64_data[1])


def test_int64_erase_ranges(table, intervals):
    inserted_int64_data = _generate_int64_ts(_start_time(intervals), 1000)
    table.int64_insert(_int64_col_name(table), inserted_int64_data[0], inserted_int64_data[1])
...
def test_blob_get_ranges(table, intervals):
    ...
    # error: column doesn't exist
    with pytest.raises(quasardb.Error):
        table.blob_get_ranges("lolilol", [(_start_time(intervals), _start_time(intervals) + np.timedelta64(10, 's'))])

    with pytest.raises(quasardb.Error):
        table.blob_insert("lolilol", inserted_blob_data[0], inserted_blob_data[1])

    with pytest.raises(TypeError):
        table.int64_get_ranges(_blob_col_name,
                               [(_start_time(intervals), _start_time(intervals) + np.timedelta64(10, 's'))])

    with pytest.raises(TypeError):
        table.int64_insert(_blob_col_name(table), inserted_blob_data[0], inserted_blob_data[1])


def test_blob_erase_ranges(table, intervals):
    inserted_blob_data = _generate_blob_ts(_start_time(intervals), 1000)
    table.blob_insert(_blob_col_name(table), inserted_blob_data[0], inserted_blob_data[1])
